...
def save_graph(folder):
    ...
    info = pd.read_csv(
        os.path.join(folder, 'progress.csv'), header=None, lineterminator ='|',
...
def save_epoch(state, mean, save_path, out_file, checkpoint, is_best):
    out_file.write('{},{},{},{},{},{},{}|'.format(datetime.now(), state['epoch'], state['loss'][0], state['loss'][1],state['dice'], mean, state['lr']))
    out_file.flush()
